...
/*
    @param _bufferSize  tamanho do buffer sem ser tranformado em bytes, em MB, fazendo a conversao dentro da funcao.
    @example FastMeasure(32)  ira fazer testes com buffers de 32 MB.
*/
void FastMeasure(const int _bufferSize = 32)
{
    printf("Busy waiting to raise the CPU frequency...\n");
    // Busy wait for a second so that the CPUs ramp up to full speed.
    BusyWait(500);
    const int bufSize = _bufferSize * 1024 * 1024;
